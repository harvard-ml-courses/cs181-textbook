...
The simplest use case for a discriminant function is when we only have two classes that we are trying to decide between. Let's denote these two classes by 1 and -1. Our discriminant function in Equation \ref{basic-discriminant-fn} will then predict class 1 if $h(\textbf{x}, \textbf{w}) \geq 0$ and class -1 if $h(\textbf{x}, \textbf{w}) < 0$:
\begin{align*}
	\begin{cases} 
...
Instead, we can avoid these ambiguities in the multi-class case by using one discriminant function with $K$ outputs $h_{k}$, where $h(\textbf{x}, \textbf{W}) = \textbf{W}\textbf{x}$ and $\textbf{W}$ is $K \times D$. Since this discriminant outputs $K$ elements, we can transform our single-valued labels $y$ to match these dimensions via \textit{one-hot encoding}. This simply means that the class of a given data point is described by a vector with $K$ coordinates that has a 1 in the position that corresponds to class $C_{k}$ and 0s everywhere else (note that these classes aren't usually 0-indexed). For example, class $C_{1}$ of 4 classes would be represented by the vector $[1, 0, 0, 0]^\top$, while class $C_{2}$ would be represented by the vector $[0, 1, 0, 0]^\top$ and so on. 

We can then assign new data points to the class $C_{k}$ for which $h_{k}(\textbf{x}, \textbf{W}) > h_{j}(\textbf{x}, \textbf{W})$ for all $j \neq k$ (and break ties if necessary). Then, similar to the two-class case, the decision boundaries are described by the surface along which $h_{k}(\textbf{x}, \textbf{W}) = h_{j}(\textbf{x}, \textbf{W})$.


\subsection{Basis Changes in Classification}
Now that we've explored the basic setup of classification problems, we can consider how to learn the weights $\textbf{w}$ that define the optimal discriminant functions in the binary case. However, prior to solving for $\textbf{w}$, we need to discuss how basis transformations apply to classification problems. We initially discussed basis changes in the context of linear regression, and they are equally important for classification tasks. For example, consider the data set in Figure \ref{fig:circles-without-basis-change}.
...
\begin{equation} \label{basis-changed-linear-model}
    h(\textbf{x}, \textbf{w}) = \textbf{w}^\top\phi{(\textbf{x})}
\end{equation}
...
Recall from the previous chapter on linear regression that when it comes to optimizing our model's weight parameters \textbf{w}, the goal is to minimize our loss function $\mathcal{L}(\textbf{w})$ (also called the objective function). We did this by taking the derivative of our loss function with respect to \textbf{w}, setting that expression equal to 0, and solving for \textbf{w}. We were previously able to perform that procedure with confidence because the least squares loss function was \textit{convex} with respect to the weight parameters, which meant it had a global solution we could solve for directly. Thus, the point of minimization for the loss function would occur where $\nabla \mathcal{L}(\textbf{w}) = 0$.

Unfortunately, it's not always the case that our loss function will be convex with respect to our weight parameters. In fact, in the next section, we will consider an loss function that is not convex, and as a result we will need a new way to optimize our parameters. Typically, when facing a non-convex loss function, we will need to resort to a \textit{numerical} procedure.

\readernote{The terms \textit{numerical} and \textit{analytical} procedures come up very frequently in machine learning literature. An analytical solution typically utilizes a closed form equation that accepts your model and input data and returns a solution in the form of optimized model parameters. On the other hand, numerical solutions are those that require some sort of iteration to move toward an ever better solution, eventually stopping once the solution is deemed `good enough'. Analytical solutions are typically more desirable than numerical solutions due to computational efficiency and performance guarantees, but they often are not possible for complex problems due to non-convexity.}
...
The high level idea behind gradient descent is as follows: we start off with an initial guess $\textbf{w}^{(0)}$ for our parameters. Then to update our parameters, we take a small step in the opposite direction of the gradient of our loss function $\mathcal{L}$ with respect to the weight parameters $\textbf{w}^{(t)}$ at time $t$. The intuition here is that the gradient represents the direction of greatest ascent, so we want to go in the opposite direction to find the minimum. Notationally, this looks like the following:
\begin{equation} \label{gradient-descent-equation}
    \textbf{w}^{(t+1)} = \textbf{w}^{(t)} - \eta \nabla \mathcal{L}(\textbf{w}^{(t)})
\end{equation}
where $\eta > 0$ is known as the \textit{learning rate}. Note that the parameter values at time $t=0$ given by $\textbf{w}^{(0)}$ are often initialized randomly.

\readernote{In general, we want a learning rate that is large enough so that we make progress toward reaching a better solution, but not so large that we take a step that puts us in a worse place in the parameter space than we were at the previous step. Notice in Figure \ref{fig:gradient-descent-step-size} that an appropriately small step size improves our loss function, while a large step size overshoots the update and leaves us in a worse position.}
...
We are already familiar with the least squares loss function from linear regression, and we can apply it again in this context to find the set of weights \textbf{w} that form the optimal decision boundary between two target classes.

Recall that the form of our linear classifier is:
\begin{equation}
	h(\textbf{x}, \textbf{w}) = \textbf{w}^\top\textbf{x}
\end{equation}
Given $N$ data points, we can then use the results derived in the last chapter to find the solution for \textbf{w} that minimizes the least squares loss function:
\begin{align*}
	\textbf{w}^{*} = (\textbf{X}^\top\textbf{X})^{-1}\textbf{X}^\top\textbf{y},
\end{align*}
where $\textbf{X}$ is $N \times D$ and $\textbf{y}$ is $N \times 1$. We can then use this $\textbf{w}^{*}$ in our discriminant function $h(\textbf{x}, \textbf{w}^{*})$ to make predictions on new data points.
...
Using the logistic sigmoid function, we now have a means of generating a probability that a new data point $\textbf{x}^{*}$ is part of class $y^{*}$. Because we are currently operating in the two-class case, which in this context will be denoted by $C_{1}=1$ and $C_{2}=0$, we'll write the probability for each of these classes as:
\begin{align*}
	p(y^{*}=1|\textbf{x}^{*}) &= \sigma(\textbf{w}^\top\textbf{x}^{*}) \\
	p(y^{*}=0|\textbf{x}^{*}) &= 1 - p(y^{*}=1|\textbf{x}^{*})
\end{align*}
...
\end{equation}
where $\hat{y}_{i} = p(y_{i}=1|\textbf{x}_{i}) = \sigma(\textbf{w}^\top\textbf{x}_{i})$.
...
Second, we will turn our log likelihood into a loss function by taking the negative of our log likelihood expression. Now, instead of maximizing the log likelihood, we will be minimizing the loss function, which will again find us the same setting of parameters.

\readernote{It's worth rereading the above paragraph again to understand the pattern presented there, which we will see several times throughout this book. Instead of maximizing a likelihood function directly, it is often easier to define a loss function using the negative log likelihood, which we can then minimize to find the optimal setting of parameters for our model.}

After taking the negative logarithm of the likelihood function defined by Equation \ref{log-reg-likelihood}, we are left with the following term, known as the \textit{cross-entropy loss function}, which we will seek to minimize:
...
With the probabilistic discriminative modeling approach, we elected to directly model $p(y^{*}|\textbf{x}^{*})$. However, there was an alternative option: we could have instead modeled the \textit{joint distribution} of the class $y^{*}$ and the input data point $\textbf{x}^{*}$ together as $p(y^{*}, \textbf{x}^{*})$. This approach is what's known as \textbf{probabilistic generative modeling} because we actually model the process by which the data was generated.
...
\begin{derivation}{Probabilistic Generative Model Likelihood Function}{model-likelihood-fn}

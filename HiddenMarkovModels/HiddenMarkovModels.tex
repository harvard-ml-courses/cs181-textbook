...
\readernote{These $\alpha$ values will be useful for
  inference   and will also be used during training a HMM via EM (in the E-step)}
...
\readernote{Notice that the base case for the $\beta$'s is 1. This is a quirk of our indexing, and it ensures we have valid messages when we pass messages back from the final state $\textbf{s}_n$.}
